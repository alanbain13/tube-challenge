-- Create derive activity state RPC function
...
BEGIN
    ...
    -- Create state version from updated_at timestamp
    state_version := extract(epoch from COALESCE(activity_rec.updated_at, activity_rec.created_at))::bigint;
    
    -- Process each station in the activity plan
    FOR station_sequence IN 1..array_length(activity_rec.station_tfl_ids, 1) LOOP
        station_id := activity_rec.station_tfl_ids[station_sequence];
        total_count := total_count + 1;
        
        -- Get station name from stations table
        SELECT name INTO station_name FROM stations WHERE tfl_id = station_id LIMIT 1;
        IF station_name IS NULL THEN
            station_name := station_id; -- fallback to ID if name not found
        END IF;
        ...
        WHERE sv.activity_id = activity_id_param 
          AND sv.station_tfl_id = station_id
        ...
        IF next_expected_sequence IS NULL AND visit_status != 'verified' THEN
            ...
            next_expected_station := station_id;
        END IF;
        
        -- Build plan item
        plan_item := jsonb_build_object(
            'sequence', station_sequence,
            'station_tfl_id', station_id,
