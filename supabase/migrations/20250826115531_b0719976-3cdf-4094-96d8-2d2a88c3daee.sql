-- Fix derive_activity_state to handle empty plans gracefully
...
BEGIN
    ...
    IF activity_rec.station_tfl_ids IS NULL
      ...
      RETURN jsonb_build_object(
        ...
        'plan', '[]'::jsonb,
        'counts', jsonb_build_object('total', 0, 'visited', 0, 'pending', 0),
        'next_expected', NULL,
        'warnings', jsonb_build_object('empty_plan', true)
      );
    END IF;
    
    -- Process each station using WITH ORDINALITY for safe iteration
