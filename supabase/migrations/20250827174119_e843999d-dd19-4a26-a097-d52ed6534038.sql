-- Fix the derive_activity_state function to properly handle visits
...
BEGIN
    ...
    IF activity_rec.station_tfl_ids IS NULL
      ...
      -- For empty plan activities, still check for actual visits
      FOR latest_visit IN
          ...
          WHERE sv.activity_id = activity_id_param 
            AND sv.status IN ('verified', 'pending')
      ...
      LOOP
          ...
          -- Only count verified visits for actual_visits
      ...
      RETURN jsonb_build_object(
        ...
        'counts', jsonb_build_object('planned_total', 0, 'visited_actual', actual_visit_count, 'pending', 0),
    ...
    -- Get actual visits in chronological order (free-order) - FIXED
    FOR latest_visit IN
        SELECT sv.station_tfl_id, sv.visited_at, sv.verification_image_url, sv.status,
               ROW_NUMBER() OVER (ORDER BY sv.visited_at) as visit_sequence
        FROM station_visits sv
    ...
    LOOP
        ...
        -- Only include verified visits in actual_visits
        IF latest_visit.status = 'verified' THEN
            actual_visit_item := jsonb_build_object(
                'sequence', latest_visit.visit_sequence,
                'station_tfl_id', latest_visit.station_tfl_id,
